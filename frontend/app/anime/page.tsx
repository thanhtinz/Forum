...
import { Search, Star, Film, BookOpen, BookText } from 'lucide-react';
...
export default function AnimeListPage() {
  ...
  const [f, setF] = useState({ type: '', genre: '', status: '', season: '', year: '', sort: 'popularity', search: '' });
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-gradient-to-r from-indigo-600 to-fuchsia-600 p-6 text-white shadow-card">
      ...
      <div className="flex flex-wrap gap-2">
        {TYPE_TABS.map((t) => (
          <button key={t.v} onClick={() => setF((s) => ({ ...s, type: t.v }))}
            className={`rounded-full px-4 py-1.5 text-sm font-medium ${f.type === t.v ? 'bg-brand-600 text-white' : 'bg-ink-100 dark:bg-ink-800'}`}>{t.label}</button>
        ))}
      </div>
      ...
      <div className="card flex flex-wrap items-center gap-2 p-3">
        <form onSubmit={submitSearch} className="flex min-w-[180px] flex-1 items-center gap-1 rounded-lg border border-ink-200 px-2 dark:border-ink-700">
          ...
          <input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Tìm tên anime/manga…" className="w-full bg-transparent py-1.5 text-sm outline-none" />
        </form>
      ...
      {loading ? (
        ...
        <p className="card p-10 text-center text-ink-500">Chưa có dữ liệu phù hợp. Admin có thể import từ AniList ở trang quản trị.</p>
      ) : (
        <>
