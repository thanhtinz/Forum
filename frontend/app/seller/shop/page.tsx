...
const VALID: Tab[] = ['info', 'products', 'coupons', 'tickets', 'earnings'];

function ManageStore() {
  const { user, loading } = useAuth();
  const initialTab = (useSearchParams().get('tab') as Tab) || 'info';
  const [tab, setTab] = useState<Tab>(VALID.includes(initialTab) ? initialTab : 'info');
...
export default function SellerShopPage() {
  return <Suspense fallback={<div className="p-10 text-center text-ink-500">Đang tải…</div>}><ManageStore /></Suspense>;
}
