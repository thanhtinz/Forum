/**
 * Seed script — Tạo Sticker Pack "Pepe" trong database.
 * Chạy: npx ts-node -P tsconfig.json scripts/seed-pepe-stickers.ts
 * Hoặc: DATABASE_URL=... npx ts-node scripts/seed-pepe-stickers.ts
 */
...
const SLUG = 'pepe';
const NAME = 'Pepe';
const THUMBNAIL = 'https://hoathinh3d.co/wp-content/uploads/wpdiscuz-stickers/pepe/696b6b7d39cc8.webp';
...
async function main() {
  const existing = await prisma.stickerPack.findUnique({ where: { slug: SLUG } });
  if (existing) {
    console.log(`Pack "${SLUG}" đã tồn tại (id: ${existing.id}). Xoá trước nếu muốn tạo lại.`);
    return;
  }
  ...
  console.log(`✅ Đã tạo pack "${pack.name}" với ${pack._count.stickers} stickers (id: ${pack.id})`);
}
