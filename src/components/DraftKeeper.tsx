...
interface Draft { t: number; v: Record<string, string[]> }
...
export function DraftKeeper({ storageKey }: { storageKey: string }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved?.v && Date.now() - saved.t < MAX_AGE_MS
        && !isEmpty(saved.v) && !same(saved.v, read(form))) {
        setDraft(saved);
      } else if (saved) {
    ...
    const onInput = () => {
      ...
      timer = setTimeout(() => {
        ...
        try {
          ...
          else store.setItem(storageKey, JSON.stringify({ t: Date.now(), v } satisfies Draft));
        } catch {
