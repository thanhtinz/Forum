  ...
  return (
    <div className="space-y-4">
      <div className="overflow-hidden rounded-2xl bg-gradient-to-r from-brand-800 to-brand-600 p-6 text-white shadow-card">
        <h1 className="text-2xl font-bold">Truyện Tranh</h1>
        <p className="mt-1 text-sm text-white/80">Kho tàng manhua, manga, manhwa — được tuyển chọn bởi Trạm GenZ.</p>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {works.map((w) => (
              <a key={w.id} href={`/comic/detail?slug=${w.slug}`} className="card group overflow-hidden p-0 transition hover:shadow-card">
...
export default function ComicPage() {
  return <Suspense fallback={<p className="p-10 text-center text-ink-500">Đang tải…</p>}><ComicContent /></Suspense>;
}
