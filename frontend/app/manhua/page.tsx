...
export default function ManhuaPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-500 p-6 text-white shadow-card">
        <h1 className="text-2xl font-bold">Manhua</h1>
        <p className="mt-1 text-sm text-white/80">Khám phá kho tàng truyện tranh Trung Quốc cung cấp bởi Trạm GenZ.</p>
      </div>
      ...
      <div className="card flex flex-wrap items-center gap-2 p-3">
        <form onSubmit={submitSearch} className="flex min-w-[180px] flex-1 items-center gap-1 rounded-lg border border-ink-200 px-2 dark:border-ink-700">
          ...
          <input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Tìm manhua…" className="w-full bg-transparent py-1.5 text-sm outline-none" />
        </form>
              ...
              <a key={w.id} href={`/anime/detail?slug=${w.slug}`} className="card group overflow-hidden p-0 transition hover:shadow-card">
                <div className="relative aspect-[3/4] bg-ink-100 dark:bg-ink-800">
                  {w.coverUrl
                    ...
                    : <span className="grid h-full place-items-center text-ink-400"><BookOpen size={28} /></span>}
                  {w.avgScore > 0 && (
                  ...
                  <span className="absolute right-1 top-1 inline-flex items-center gap-0.5 rounded bg-black/60 px-1.5 py-0.5 text-[10px] text-white"><BookOpen size={10} /> {w.format || 'Manhua'}</span>
                </div>
                <div className="p-2">
                  <p className="line-clamp-2 text-sm font-medium leading-tight" title={w.title}>{w.titleEnglish || w.title}</p>
                  <p className="mt-0.5 text-[11px] text-ink-400">{w.seasonYear || ''}{w.chapters ? ` · ${w.chapters} ch.` : ''}</p>
                </div>
              </a>
            ))}
          </div>
