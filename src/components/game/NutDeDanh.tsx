...
import { doiDeDanh } from '@/app/(cua-hang)/game/[duongDan]/viec-de-danh';

/**
 * Nút ĐỂ DÀNH — trái tim/đánh dấu của trang game.
 *
 ...
 * chia sẻ của hệ điều hành trên máy bàn, và để dành là việc người ta làm
 ...
 */
export function NutDeDanh({ gameId, duongDan, banDau, daDangNhap }: {
  gameId: string;
  duongDan: string;
  banDau: boolean;
  daDangNhap: boolean;
  ...
  const bam = () => {
    ...
    datBat(doan);
    batDau(async () => {
      const kq = await doiDeDanh(gameId);
      if (kq.loi) { datBat(!doan); return; }
      if (typeof kq.deDanh === 'boolean') datBat(kq.deDanh);
    });
  };

  return (
    <button type="button" onClick={bam} disabled={dangGui} className="nut-tron"
      aria-pressed={bat} aria-label={bat ? 'Bỏ khỏi danh sách để dành' : 'Để dành game này'}
      title={bat ? 'Đã để dành' : 'Để dành'}>
