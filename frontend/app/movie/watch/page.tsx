...
function VideoPlayer({ url, referer, isHls, introEnd, showNextAt, episodeDurationMin, skipIntro, autoNext, onNextAt, onEnded }: PlayerProps & { isHls: boolean }) {
  ...
  useEffect(() => {
    ...
    Promise.all([import('artplayer'), isHls ? import('hls.js') : Promise.resolve(null)]).then(([artMod, hlsMod]) => {
      ...
      art = new Artplayer({
        container: el, url: src, type: isHls ? 'm3u8' : 'mp4',
        autoplay: true, setting: true, playbackRate: true, aspectRatio: true,
        fullscreen: true, fullscreenWeb: true, pip: true, miniProgressBar: true,
        autoOrientation: true, theme: '#6366f1',
        controls: [
          { position: 'left', name: 'backward10', html: `<svg xmlns="http://www.w3.org/2000/svg" height="22" width="22" viewBox="0 0 24 24" fill="white"><path d="M11.99 5V1l-5 5 5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6h-2c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8zm-1.1 11H9.9v-3.26L9 12.16l.77-.54.88-.62V16zm4.17 0h-2.89v-.88h.98v-3.3h-.96v-.85c.32-.05.56-.14.73-.26.17-.13.29-.29.36-.51h.89V15.12h.89V16z"/></svg>`, tooltip: 'Lùi 10 giây', click: function() { art.backward = 10; } },
          { position: 'left', name: 'forward10', html: `<svg xmlns="http://www.w3.org/2000/svg" height="22" width="22" viewBox="0 0 24 24" fill="white"><path d="M18 13c0 3.31-2.69 6-6 6s-6-2.69-6-6 2.69-6 6-6v4l5-5-5-5v4c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8h-2zm-4.5 3h-1.1v-3.26l-.9-.58.77-1.16 1.23.82V16zm3.17 0H13.78v-.88h.98v-3.3h-.96v-.85c.32-.05.56-.14.73-.26.17-.13.29-.29.36-.51h.89v4.92h.89V16z"/></svg>`, tooltip: 'Tới 10 giây', click: function() { art.forward = 10; } },
        ],
        customType: isHls
          ? { m3u8: (video: HTMLVideoElement, u: string, instance: any) => {
              ...
              if (Hls && Hls.isSupported()) {
                ...
                hls.on(Hls.Events.ERROR, (_e: any, data: any) => {
                  ...
                  if (data.type === Hls.ErrorTypes.NETWORK_ERROR && !viaProxyRef.current) { hls.destroy(); setViaProxy(true); return; }
          ...
          : { mp4: (video: HTMLVideoElement, u: string) => { (video as any).referrerPolicy = 'no-referrer'; video.src = u; } },
      });
      art.on('video:timeupdate', () => {
        const { introEnd, skipIntro } = introRef.current;
        if (skipIntro && introEnd && !skippedRef.current && art.currentTime < introEnd) { skippedRef.current = true; art.currentTime = introEnd; }
        if (!nextAtFiredRef.current) {
          ...
          const triggerAt =
            ...
            : null;
          if (triggerAt != null && art.currentTime >= triggerAt) { nextAtFiredRef.current = true; nextRef.current.onNextAt(); }
        }
      });
      let endedFired = false;
      nativeEndedHandler = () => {
        if (endedFired) return; endedFired = true;
      ...
      art.on('error', () => { if (!isHls && !viaProxyRef.current) { setViaProxy(true); return; } setError('Trình duyệt không tải được (CORS/403).'); });
...
function IframePlayer({ url, showNextAt, episodeDurationMin, autoNext, onNextAt, onEnded }: PlayerProps) {
  ...
  useEffect(() => {
    function onMsg(e: MessageEvent) {
      try {
        ...
        let d: any = raw;
        if (typeof raw === 'string') { try { d = JSON.parse(raw); } catch { d = raw; } }
        const str = typeof d === 'string' ? d.toLowerCase() : '';
        if (d?.event === 'onStateChange' && (d?.info === 0 || d?.info === '0')) { if (nextRef.current.autoNext) nextRef.current.onEnded(); return; }
        const isEnded =
          str === 'ended' || d?.event === 'ended' || d?.type === 'ended' || d?.action === 'ended' ||
          d?.status === 'ended' || d?.state === 'ended' || d?.event === 'video:ended' || d?.event === 'complete' ||
  ...
  const yt = ytId(url);
  if (yt) return <iframe src={`https://www.youtube.com/embed/${yt}?autoplay=1&enablejsapi=1`} className="h-full w-full" allowFullScreen title="Player" />;
...
function MovieWatch() {
  ...
  useEffect(() => {
    ...
    if (nextCountdown <= 0) { if (autoNextRef.current && ep?.next) goNext(); setNextCountdown(null); return; }
  ...
  const tabGroups = useMemo(() => {
    ...
    for (const e of allEps) {
      if (!e.kind || e.kind === 'episode') { const p = e.part ?? 1; if (!partMap.has(p)) partMap.set(p, []); partMap.get(p)!.push(e); }
      else { singles.push(e); }
    }
    [...partMap.entries()].sort((a, b) => a[0] - b[0]).forEach(([part, items]) => { groups.push({ key: `part-${part}`, label: `Phần ${part}`, kind: 'part', items }); });
  ...
  function goNext() {
    if (!ep?.next) return;
    setNextCountdown(null); setNextDismissed(true);
    router.push(`/movie/watch?ep=${ep.next.id}`);
  }
  function handleNextAt() { if (nextDismissed || nextCountdown !== null || !ep?.next) return; setNextCountdown(15); }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4">
      <a href={`/movie/detail?slug=${ep.media.slug}`} className="inline-flex items-center gap-1 text-sm text-ink-500 hover:text-brand-600"><ArrowLeft size={15} /> {ep.media.title}</a>
      ...
      <div className="overflow-hidden rounded-xl bg-black shadow-card">
        ...
        <div className="grid grid-cols-5 divide-x divide-white/10 border-t border-white/10 bg-ink-900 text-white">
          ...
          <a href={ep.prev ? `/movie/watch?ep=${ep.prev.id}` : undefined} className={`flex flex-col items-center gap-0.5 py-2 text-[10px] ${ep.prev ? 'hover:bg-white/5' : 'opacity-40'}`}><SkipBack size={17} /> Trước</a>
          <a href={ep.next ? `/movie/watch?ep=${ep.next.id}` : undefined} className={`flex flex-col items-center gap-0.5 py-2 text-[10px] ${ep.next ? 'hover:bg-white/5' : 'opacity-40'}`}><SkipForward size={17} /> Tiếp</a>
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {tabGroups.map((g) =>
                g.kind === 'part' ? (
                  ...
                  <a key={g.key} href={`/movie/watch?ep=${g.epId}`}
            ...
            <>
              ...
              <div className="grid max-h-72 grid-cols-5 gap-1.5 overflow-y-auto sm:grid-cols-6">
                {tabEpisodes.map((e: any) => (
                  <a key={e.id} href={`/movie/watch?ep=${e.id}`}
      ...
      <div className="card p-5">
        ...
        {user ? (
          <div className="mb-4">
            <CommentBox user={user} onSubmit={(content) => postComment(content)} posting={posting} placeholder="Viết bình luận về tập này…" />
          </div>
        ) : (
...
interface CommentNodeProps {
  c: CommentT; depth: number; user: any; isMod: boolean;
  ...
  replyPosting: boolean; onDel: (id: string) => void; onReply: (content: string, parentId: string) => void;
}
    ...
    <div className={indent > 0 ? 'ml-6 border-l-2 border-ink-200 pl-3 dark:border-ink-700' : ''}>
      ...
      {user && (
        <div className="ml-9 mt-1">
          {replyingToId !== c.id && <button onClick={() => setReplyingToId(c.id)} className="text-[11px] text-ink-400 hover:text-brand-600">Trả lời</button>}
          {replyingToId === c.id && (
            <div className="mt-1.5 flex items-start gap-2">
              <Avatar user={user} size={24} />
              <div className="flex-1">
                <div className="relative">
                  <textarea autoFocus rows={2} value={replyTexts[c.id] || ''} onChange={(e) => setReplyTexts((t) => ({ ...t, [c.id]: e.target.value }))}
                    placeholder={`Trả lời @${c.author.displayName || c.author.username}…`} className="input w-full resize-none pr-8 text-sm" />
                  <button type="button" onClick={() => setReplyPicker((v) => !v)} className={`absolute right-2 top-2 rounded p-0.5 hover:bg-ink-100 dark:hover:bg-ink-700 ${replyPicker ? 'text-brand-600' : 'text-ink-400'}`}><Smile size={15} /></button>
                  {replyPicker && <EmojiStickerPicker onEmoji={(e) => setReplyTexts((t) => ({ ...t, [c.id]: (t[c.id] || '') + e }))} onSticker={(url) => { setReplyPicker(false); onReply(url, c.id); }} onClose={() => setReplyPicker(false)} />}
                </div>
                <div className="mt-1 flex gap-2">
                  <button disabled={replyPosting || !(replyTexts[c.id] || '').trim()} onClick={() => onReply(replyTexts[c.id] || '', c.id)} className="inline-flex items-center gap-1 rounded-lg bg-brand-600 px-3 py-1 text-xs text-white hover:bg-brand-700 disabled:opacity-50"><Send size={12} /> Gửi</button>
      ...
      {(c.replies || []).length > 0 && (
        <div className="mt-2 space-y-2">
          {(c.replies || []).map((r) => <CommentNode key={r.id} c={r} depth={depth + 1} user={user} isMod={isMod} replyingToId={replyingToId} setReplyingToId={setReplyingToId} replyTexts={replyTexts} setReplyTexts={setReplyTexts} replyPosting={replyPosting} onDel={onDel} onReply={onReply} />)}
        </div>
      )}
    </div>
  );
...
export default function MovieWatchPage() {
  return <Suspense fallback={<p className="p-10 text-center text-ink-500">Đang tải…</p>}><MovieWatch /></Suspense>;
}
