...
export default function AdminFortune() {
  ...
  async function fetchModels() {
    ...
    try { const r = await api.post<{ models: string[]; error?: string }>('/ai/models', { provider: cfg.aiProvider, apiKey: apiKey || undefined }); setModels(r.models || []); if (r.error) setMsg(`Tải model lỗi: ${r.error}`); else if (!r.models?.length) setMsg('Không lấy được model — lưu API key trước.'); }
