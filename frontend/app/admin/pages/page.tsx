      ...
      {tab === 'pages' && (
        <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
            ...
            {pages.map((p) => (
              <div key={p.id} className="card flex items-center justify-between p-3">
                <div className="min-w-0">
                  <div className="truncate font-medium">{p.title} <span className="text-xs text-ink-400">/p?slug={p.slug}</span></div>
          ...
          <div className="card space-y-2 p-4">
            ...
            <textarea className="input resize-y" rows={8} placeholder="Nội dung (Markdown)…" value={pForm.content || ''} onChange={(e) => setPForm({ ...pForm, content: e.target.value })} />
