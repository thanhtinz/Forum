  ...
  return (
    <div className="card p-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <Link href="/conversations" className="flex items-center gap-2 rounded-lg p-2 text-sm hover:bg-ink-100 dark:hover:bg-ink-800"><Mail size={16} className="text-violet-600" /> Tin nhắn</Link>
      </div>
    </div>
  );
}
...
const WIDGET_MAP: Record<string, () => JSX.Element | null> = {
  community: CommunityWidget,
  stats: StatsWidget,
  trending: TrendingWidget,
  newMembers: NewMembersWidget,
};

export function HomeSidebar() {
  const cfg = useSiteConfig();
  const order = cfg.homeWidgets.filter((w) => w.enabled && WIDGET_MAP[w.key]);

  return (
    <aside className="space-y-4">
      {order.map((w) => {
        const Widget = WIDGET_MAP[w.key];
        return <Fragment key={w.key}><Widget /></Fragment>;
      })}
    </aside>
  );
}
