    ...
    <div className="space-y-4 pb-8">
      ...
      {w.genres?.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {w.genres.map((g: any) => (
            <a key={g.slug} href={`/truyen-tranh?genre=${g.slug}`}
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        {lastCh ? (
          <a href={`/manga/read?id=${lastCh.id}`}
...
export default function MangaDetailPage() {
  return <Suspense fallback={<div className="p-10 text-center text-ink-500">Đang tải…</div>}><MangaDetail /></Suspense>;
}
