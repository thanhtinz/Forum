...
import { BAC_TOI_DA, CANH_GIOI, DAO, LINH_CAN, SO_TANG, TANG_TEN, tuViCanDe } from '@/lib/tu-tien-const';
    ...
    <>
      ...
      <section className="tien-tam p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {CANH_GIOI.map((c) => (
                <tr key={c.bac} className={cn('border-b', c.bac === nv.bac && 'tien-dao-mau font-bold')}
        ...
        <ul className="tien-so mt-3 space-y-1 text-sm">
          {CANH_GIOI.map((c) => (
            <li key={c.bac}>
              ...
              {/*
                Tầng cuối của bậc cuối là TRẦN của giai đoạn này nên
                `tuViCanDe` trả 0 — in thẳng số 0 ra thì đọc thành "tầng chót
                miễn phí", ngược hẳn ý.
              */}
              {c.bac >= BAC_TOI_DA
                ? ' tu vi, tới tầng chót là hết đường của giai đoạn này.'
      ...
      <section className="tien-tam p-5">
        ...
        <ul className="space-y-3 text-sm">
          {DAO.map((d) => (
            <li key={d.ma} className={cn('border-b pb-2', d.ma === nv.dao && 'tien-dao-mau')}
              style={{ borderColor: 'var(--tien-vien)' }}>
        ...
        <ul className="grid gap-x-5 gap-y-1 text-sm sm:grid-cols-2">
          {LINH_CAN.map((l) => (
            <li key={l.id} className={cn('flex justify-between gap-2 border-b pb-0.5',
              l.id === nv.linhCan && 'tien-dao-mau font-bold')}
              style={{ borderColor: 'var(--tien-vien)' }}>
