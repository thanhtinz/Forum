...
/**
 * Phòng chat chung phải chặn thật ở phía máy chủ.
 *
...
export default async function run(check) {
  ...
  // ── Khách ──────────────────────────────────────────────────────────────
  const gRes = await guest.goto(`${BASE}/chat`, { waitUntil: 'networkidle' });
  check('khách mở được phòng chat', gRes.status() === 200);
  check('khách không có ô nhập', (await guest.locator('input[name="content"]').count()) === 0);
  check(
  ...
  // ── Nói một câu ────────────────────────────────────────────────────────
  await minh.goto(`${BASE}/chat`, { waitUntil: 'networkidle' });
  await minh.waitForTimeout(500);
  await minh.fill('input[name="content"]', 'Câu kiểm thử phòng chat');
  await minh.click('button[type="submit"]');
  ...
  await minh.click('button[type="submit"]');
  await minh.waitForTimeout(1800);
  check('câu 400 ký tự bị máy chủ từ chối', (await db.shoutMessage.count({ where: { content: long } })) === 0);

  // ── Nói liên tiếp bị chặn ──────────────────────────────────────────────
  await lan.goto(`${BASE}/chat`, { waitUntil: 'networkidle' });
  await lan.waitForTimeout(500);
  await lan.fill('input[name="content"]', 'câu một');
  await lan.click('button[type="submit"]');
  await lan.waitForTimeout(900);
  await lan.fill('input[name="content"]', 'câu hai ngay lập tức');
  await lan.click('button[type="submit"]');
  ...
  check(
    ...
    (await lan.locator('button[title="Gỡ câu này"]').count()) ===
  ...
  await lan.locator('button[title="Gỡ câu này"]').first().click();
