...
export default async function run(check) {
  ...
  // Dọn trong `finally`: bài này dựng một chuyên mục KHOÁ HUY HIỆU, mà mười bốn
  // bài khác lấy chuyên mục bằng `findFirst` theo `postAccess: 'ALL'`. Đứt giữa
  // chừng mà không dọn thì cái khu khoá ấy nằm lại, bài sau bốc trúng nó rồi đỏ
  // hàng loạt vì trang chủ đề dựng ra bản bị chắn. Nó cũng nâng `minhdev` lên
  // MODERATOR ở đoạn cuối — bỏ quên là lệch quyền cho cả bộ kiểm.
  try {
    ...
    await db.user.updateMany({ where: { id: a.id }, data: { role: 'USER' } });
    await db.thread.deleteMany({ where: { forumId: forum.id } });
    await db.forum.deleteMany({ where: { id: forum.id } });
    await db.medal.deleteMany({ where: { id: medal.id } });
  }
}
