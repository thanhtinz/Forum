  ...
  return (
    <>
      ...
      {!expanded ? (
        ...
        <div className="fixed right-2 top-1/2 z-40 flex -translate-y-1/2 flex-col items-center gap-2 rounded-2xl bg-brand-600 p-2 shadow-2xl">
          <button onClick={() => setExpanded(false)} title="Thu gọn" className="grid h-7 w-7 place-items-center rounded-full text-white/80 hover:bg-white/15">
            <ChevronRight size={16} />
          </button>
          {buttons.map((b) => (
            <button key={b.key} onClick={() => choose(b.key)} title={b.title}
              className={`grid h-11 w-11 place-items-center rounded-full text-white shadow-md transition active:scale-95 ${b.cls} ${active === b.key ? 'ring-2 ring-white ring-offset-2 ring-offset-brand-600' : ''}`}>
              <b.icon size={20} />
            </button>
          ))}
        </div>
