...
import {
  Briefcase, Clock, Users2, Star, Send, ShieldCheck, AlertTriangle,
  CheckCircle2, RotateCcw, Upload, Gavel, Tags,
} from 'lucide-react';
...
// Bảng điều khiển "Việc làm" hiển thị inline trong trang xem bài (thread),
// khi bài thuộc danh mục được đánh dấu là module JOB.
export default function ThreadJobPanel({ threadId }: { threadId: string }) {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const j = await api.get<Job | null>(`/jobs/by-thread/${threadId}`);
      setJob(j);
      if (j?.isOwner) api.get<Proposal[]>(`/jobs/${j.id}/proposals`).then(setProposals).catch(() => {});
    } catch (e: any) { setErr(e.message); }
    finally { setLoading(false); }
  }, [threadId]);

  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { load(); }, [threadId, user]);
  ...
  return (
    <div className="space-y-4">
      {/* Tổng quan việc làm */}
      <div className="card border-emerald-300 p-5 dark:border-emerald-800">
        <div className="flex flex-wrap items-center gap-2">
          <span className="chip bg-emerald-100 text-emerald-700"><Briefcase size={12} className="mr-1 inline" /> Việc làm</span>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-ink-500">
          ...
          {job.deadline && <span className="flex items-center gap-1"><Clock size={14} /> hạn {new Date(job.deadline).toLocaleDateString('vi-VN')}</span>}
        </div>
        <div className="mt-3 flex flex-wrap items-center gap-4">
          <div>
            <div className="text-lg font-semibold text-emerald-600">{formatBudget(job)}</div>
            <div className="text-xs text-ink-500">{BUDGET_TYPE_LABELS[job.budgetType] || job.budgetType}</div>
          </div>
          {job.escrow && (
